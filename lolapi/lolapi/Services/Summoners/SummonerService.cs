...
public class SummonerService : ISummonerService{
    ...
    public string[] GetSummoner(Summoner summoner){
        try{ 
            SummonerList summonerList = GetSummoner("summoners", summoner.summoner).Result;
            List<Cluster> clusters = GetClusters().Result;
            string[] recommendation = new Recommendation(summonerList.mList, clusters).recommendation;
            return recommendation;
        }
        catch (Exception e){
            Console.WriteLine("Error in GetSummoner");
            throw e;
        }
